...
访问地址：https://records.winney07.cn/
...
1. 在GitHub中新建records仓库

2. 克隆仓库

   ```
   git clone git@github.com:winney07/records.git
   ```
...
8. 在本地启动服务器
   ...
   `http://localhost:8080` 默认访问到的页面是`records/docs/README.md`的内容
...
11. [默认主题配置](https://www.vuepress.cn/theme/default-theme-config.html)，以下代码放在根级 `README.md`，即`records/docs/README.md`
...
2. 修改base
   ...
   // .vuepress/config.js
   module.exports = {
       base:'/records/',   // 仓库中的目录名
   }
   ```

   `注：改了base之后，http://localhost:8080/records，这样可以访问原来的页面

3. 新增部署文件(deploy.sh)

   `docs/deploy.sh`：在records目录下新建`deploy.sh`
   ...
   `提交代码后，可以在records仓库中，看到gh-pages分支`

4. 验证`gh-pages`

   1. 点击仓库的`Settings`，查看`GitHub Pages`，可以看到GitHub Pages已经启用
   2. 访问https://winney07.github.io/records/即可

5. 设置website,便于他人访问

#### 提交项目代码到GitHub

注：要在records根目录添加`.gitignore`文件：
...
```
yangyanyi@yangyanyideMacBook-Pro records % git add .
...
2. DNS解析
   ...
   3. 在GitHub的records仓库中，Settings—Pages—Github Pages—Custom domain查看是否有成功
